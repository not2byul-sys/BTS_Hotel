...
BTS_Hotel/
├── Frontend (웹 UI)
│   ├── index.html              # 메인 웹 인터페이스 (모바일 최적화)
│   ├── app.js                  # 프론트엔드 로직 (Supabase 연동)
│   └── style.css               # 스타일시트 (모바일 우선 디자인)
│
├── Backend (데이터 수집)
│   ├── korean_ota_scraper.py   # 멀티 플랫폼 OTA 스크래퍼
│   ├── agoda_scraper.py        # Agoda 전용 스크래퍼
│   ├── availability_tracker.py # 재고 모니터링 및 알림
│   ├── run_scraper.py          # 데이터 엔진 v5.0 (메인 오케스트레이터)
│   └── app.py                  # Streamlit 대시보드
│
├── Data (JSON 데이터)
│   ├── korean_ota_hotels.json         # 메인 호텔 데이터셋 (48개 호텔)
│   ├── concert_hotels_sample.json     # 샘플 데이터 (서울/고양)
│   ├── availability_state.json        # 현재 재고 상태
│   └── notification_log.json          # 알림 발송 기록
│
├── Database
│   └── supabase_setup.sql      # PostgreSQL 스키마
│
├── Automation (자동화)
│   ├── auto_scrape.sh          # MacBook crontab 스크래퍼
│   ├── auto_update.sh          # 자동 업데이트 스크립트
│   └── .github/workflows/
│       ├── main.yml            # 수동 데이터 업데이트
...
| CSS 파일 | 1개 |
| HTML 파일 | 1개 |
| JSON 데이터 파일 | 4개 (~500KB) |
| 호텔 수 | 48개 |
| 지원 공연장 | 6개 이상 |
| 알림 채널 | 3개 (Email, Discord, Telegram) |
...
3. **지능형 알림** - Discord, Telegram, Email 알림
